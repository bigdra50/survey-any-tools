...
空なら §1.5 のリモートフォールバックに切り替える (ghq / mise が無い環境でも動かすため)。
リモートも使えない場合のみエラー終了し、ユーザーに `ghq get` を促す。

### 1.5 リモートフォールバック (checkout が無い環境)

ローカル checkout / mise が使えないときは、読み取り API を直接叩く。
token は `~/.config/survey-any/.env` の `PROGRESS_TOKEN` (または環境変数)。

```bash
TOKEN=$(sed -n 's/^PROGRESS_TOKEN=//p' ~/.config/survey-any/.env 2>/dev/null || echo "$PROGRESS_TOKEN")
BASE="${SURVEY_ANY_URL:-https://survey-any.pages.dev}"

# fm 相当 (kind: topic|reference|wiki|course|lesson)
curl -s -H "Authorization: Bearer $TOKEN" "$BASE/api/fm?kind=topic"
# search-fulltext 相当 (BM25 のみ。--hybrid は無い)
curl -s -H "Authorization: Bearer $TOKEN" "$BASE/api/search?q=<query>&top=10&kind=topic"
# Read 相当 (lesson は /api/content/lesson/<course>/<NN-slug>)
curl -s -H "Authorization: Bearer $TOKEN" "$BASE/api/content/topic/<name>"
curl -s -H "Authorization: Bearer $TOKEN" "$BASE/api/content/reference/<name>?format=raw"
```

リモート経路の制約 (該当する動線はスキップし、回答にその旨を 1 行添える):

- `trace` 記録・`fm-related`・`cited-by`・`cites` は未対応 (ローカル専用)
- 検索は BM25 のみ (dense/hybrid なし)。スコア値はローカルと非互換
- index は main への push 時に同期される (未 push の変更は検索に出ない)
