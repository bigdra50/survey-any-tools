...
| レベル | 判定基準 | コース構成への反映 |
|--------|----------|--------------------|
| beginner | 「わからない」が過半数 | 前提概念の導入レッスンから開始。用語集を厚めに。**足場（例・反例・図）を厚く配置** |
| intermediate | 正答 / 部分正答が半分前後 | 前提は軽く復習、実装レッスンに重点を置く。足場は標準量 |
| advanced | ほぼ正答 | 前提をスキップ。応用・ハマりどころ・比較検討に集中。**足場は最小限にする** |

足場の量はレベルに応じて可変にする（熟達度逆転効果, Kalyuga et al. 2003）。
初学者には効く手厚い足場（冗長な例示・反例・図解の並置）が、上級者には冗長性として学習を阻害する。
診断結果を「レッスンの厚み」だけでなく「足場の密度」にも反映すること。
...
- 各レッスンの objectives（3 項目以内。**改訂 Bloom 分類（Anderson & Krathwohl 2001）の動詞で書く**。
  例: 「説明できる (understand)」「適用できる (apply)」「比較できる (analyze)」「評価できる (evaluate)」。
  各 objective は末尾の「理解度チェック」の設問 1 つと 1 対 1 で対応させる（構成的アラインメント, Biggs 1996）。
  対応が取れない objective / 設問は残さない）
- つまずきやすく変容的な**閾値概念（threshold concept, Meyer & Land 2003）** を含むレッスンには印を付け、
  そのレッスンだけ例・反例・図による足場を意図的に厚くする
...
5. **類似レッスンをブロック化しない**（interleaving, Rohrer & Taylor 2007）。同じ topics/references
   に依拠するレッスンが連続する場合も、間に別カテゴリのレッスンを挟むか演習を混在させて交互配置にする
