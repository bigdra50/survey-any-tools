...
Status は Proposed。実装着手前に本 ADR を Accepted に更新する。
...
3. **Phase 3: コンテンツ repo 分離**
   `topics/ references/ courses/ inbox/ memory/ wiki/ vocab/tags.yml` を `survey-content` repo（private）へ移す。pre-commit hook（link-papers/backlinks/index）も content 側へ。viewer の `content.config.ts` base を `SURVEY_ANY_ROOT` 由来に変更。
4. **Phase 4: 配布経路の切り替え**
   スキル SKILL.md を `survey-any <cmd>`（CLI）ベースに更新。README/CLAUDE を「uv tool install（ツール）+ content repo clone（データ）」に。apm パッケージはスキル定義のみに純化。
...
1. `vocab/tags.yml` はツール側（語彙スキーマ）とコンテンツ側（実タグはデータ依存）のどちらへ置くか。→ tags.yml は「登録済みタグの辞書」でデータに追従するため content 側が有力だが、tags-validate はツール。要検討。
2. ビューアは CLI 統合（`survey-any serve`）か独立 repo か。
3. CLI の配布は PyPI 公開か git+uv（private 維持）か。
