...
学術論文の再帰的サーベイ。落合フォーマットで各論文をまとめ、引用グラフを飽和するまで探索する。
...
```bash
mise run new-report <topic-name>
```

### 4. 論文の読み込みと落合フォーマットまとめ
...
#### 4b. 落合フォーマットで reference 作成

```bash
mise run new-paper-reference <name>
```
...
```
論文「{title}」を読んで落合フォーマットでまとめてください。
...
2. `mise run new-paper-reference {ref_name}` で reference ファイルを作成
3. 落合フォーマットの6項目を埋める
...
作業ディレクトリ: /Volumes/CrucialX9/dev/github.com/bigdra50/survey-any
```
