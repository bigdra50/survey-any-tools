...
- cwd が survey-any リポジトリ (project scope skill のため自動的にその前提)。
- `bun` / `wrangler` が利用可能 (viewer/ の依存)。
...
`mise run preview` をバックグラウンドで起動する (サーバは起動したまま常駐するため)。
...
enum 自体を拡張する場合は SoT 2 箇所 (+必要なら skill 文書) を揃える:

1. `viewer/src/content.config.ts` の zod `z.enum([...])`
2. `scripts/_schema.py` の対応する `frozenset({...})`
3. 値の意味を文書化しているスキル (例: read_depth は `skills/survey-paper/SKILL.md`)
...
| タスク | 用途 | 反映先 |
|---|---|---|
| `mise run preview` | ローカル確認 | `localhost:8788` (wrangler pages dev) |
| `mise run deploy` | 本番デプロイ | Cloudflare Pages |
